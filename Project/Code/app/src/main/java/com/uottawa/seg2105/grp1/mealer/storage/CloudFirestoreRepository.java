...
public final class CloudFirestoreRepository implements IRepository {
    ...
    @Override
    public <T extends IDatabaseEntity> boolean hasId(Class<T> cls, String id) throws RepositoryRequestException {
        try {
            // Block until the DocumentSnapshot can be retrieved
            Task<DocumentSnapshot> task_docsnap = db.collection(IDatabaseEntity.getTableName(cls)).document(id).get();
    ...
    @Override
    public <T extends IDatabaseEntity> T getById(Class<T> cls, String id) throws RepositoryRequestException {
        try {
            // Block until the DocumentSnapshot can be retrieved
            Task<DocumentSnapshot> task_docsnap = db.collection(IDatabaseEntity.getTableName(cls)).document(id).get();
            DocumentSnapshot docsnap = Tasks.await(task_docsnap);
    ...
    @Override
    public <T extends IDatabaseEntity> void set(Class<T> cls, T entity) throws RepositoryRequestException {
        try {
            // Attempt to serialise the Entity
            Map<String, Object> serialised = entity.serialise();

            // Block until the document has been set
            Task<Void> task_setdoc = db.collection(IDatabaseEntity.getTableName(cls)).document(entity.getId()).set(serialised);
    ...
    @Override
    public <T extends IDatabaseEntity> void update(Class<T> cls, String id, Map<String, Object> properties) throws RepositoryRequestException {
        try {
            // Block until the file is updated
            Task<Void> task_update = db.collection(IDatabaseEntity.getTableName(cls)).document(id).update(properties);
    ...
    @Override
    public <T extends IDatabaseEntity> void delete(Class<T> cls, T entity) throws RepositoryRequestException {
        try {
            // Block until the file is deleted
            Task<Void> task_delete = db.collection(IDatabaseEntity.getTableName(cls)).document(entity.getId()).delete();
    ...
    @Override
    public <T extends IDatabaseEntity> List<T> list(Class<T> cls) throws RepositoryRequestException {
        try {
            // Block until all elements have been returned
            Task<QuerySnapshot> task_querysnap = db.collection(IDatabaseEntity.getTableName(cls)).get();
    ...
    @Override
    public <T extends IDatabaseEntity> List<T> query(Class<T> cls, Predicate<T> predicate) throws RepositoryRequestException {
        try {
            // Block until all elements have been returned
            Task<QuerySnapshot> task_querysnap = db.collection(IDatabaseEntity.getTableName(cls)).get();
            QuerySnapshot querysnap = Tasks.await(task_querysnap);
